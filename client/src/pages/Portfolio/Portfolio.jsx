...
export default function Portfolio() {
  const { portfolioSummary, balance, setBalance } = useContext(AppContext);
  ...
  return (
    <div className="portfolio-page">
      ...
      <div className="portfolio-value-card card">
        <div className="portfolio-value-main">
          <div>
            ...
            <div className="portfolio-value-amount">ETB {portfolioSummary.value.toLocaleString()}</div>
            <div className="portfolio-value-gain">+ETB{portfolioSummary.gain} ({portfolioSummary.gainPercent}%)</div>
          </div>
          <div className="portfolio-value-actions">
            <button className="btn btn-outline" onClick={() => { setShowAddFund(true); resetForm(); }}><i className="fas fa-plus"></i> Add Fund</button>
            <button className="btn btn-outline" onClick={() => { setShowWithdraw(true); resetForm(); }}><i className="fas fa-arrow-up"></i> Withdraw</button>
          </div>
        </div>
        <div style={{ display: 'flex', gap: 16, marginTop: 16 }}>
          <div className="summary-card" style={{ flex: 1 }}>
            <div className="summary-title">Total Investment</div>
            <div className="summary-value">ETB {portfolioSummary.investment.toLocaleString()}</div>
          </div>
          <div className="summary-card" style={{ flex: 1 }}>
          ...
          <div className="summary-card" style={{ flex: 1 }}>
            ...
            <div className="summary-value positive">+ ETB {portfolioSummary.todayReturn.toLocaleString()}</div>
          </div>
        </div>
      </div>
      <div className="portfolio-assets-card card">
        ...
        <div className="portfolio-assets-table-wrapper">
          <table className="portfolio-assets-table">
            ...
            <tbody>
              {/* No assets in screenshot, so leave empty for now */}
            </tbody>
          </table>
        </div>
      </div>
      ...
      {showAddFund && (
        <div className="modal">
          <div className="modal-content advanced-modal" style={{ minWidth: 400, maxWidth: 420, padding: '0', overflow: 'visible', position: 'relative', background: 'var(--card-bg)', border: '1.5px solid var(--border-color)', boxShadow: '0 4px 24px 0 var(--shadow-color)' }}>
            <button className="modal-close" style={{ position: 'absolute', top: 18, right: 18, fontSize: 28, background: 'none', border: 'none', color: '#888', cursor: 'pointer', zIndex: 2 }} onClick={() => { setShowAddFund(false); resetForm(); }}>&times;</button>
      ...
      {showWithdraw && (
        <div className="modal">
          <div className="modal-content advanced-modal" style={{ minWidth: 400, maxWidth: 420, padding: '0', overflow: 'visible', position: 'relative', background: 'var(--card-bg)', border: '1.5px solid var(--border-color)', boxShadow: '0 4px 24px 0 var(--shadow-color)' }}>
            <button className="modal-close" style={{ position: 'absolute', top: 18, right: 18, fontSize: 28, background: 'none', border: 'none', color: '#888', cursor: 'pointer', zIndex: 2 }} onClick={() => { setShowWithdraw(false); resetForm(); }}>&times;</button>
            <div style={{ padding: '36px 36px 0 36px', textAlign: 'center' }}>
              <div style={{ fontWeight: 800, fontSize: '1.6rem', marginBottom: 6, letterSpacing: 0.01 }}>Withdraw Funds</div>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(25, 167, 242, 0.07)', borderRadius: 10, padding: '10px 0', marginBottom: 18 }}>
                <i className="fas fa-wallet" style={{ color: '#1976d2', fontSize: 22, marginRight: 10 }}></i>
                <span style={{ color: '#1976d2', fontWeight: 700, fontSize: '1.13rem' }}>Available Balance:</span>
                <span style={{ color: '#1976d2', fontWeight: 800, fontSize: '1.13rem', marginLeft: 8 }}>ETB {balance.toLocaleString()}</span>
              </div>
            </div>
            <div style={{ padding: '0 36px 32px 36px' }}>
              {fundSuccess ? (
                <div className="modal-success" style={{ margin: '2.5rem 0 1.5rem 0', textAlign: 'center' }}><i className="fas fa-check-circle"></i> Withdrawal successful!</div>
              ) : (
                <form onSubmit={handleWithdrawSubmit} autoComplete="off" style={{ display: 'flex', flexDirection: 'column', gap: 22 }}>
                  {/* Amount Input */}
                  <div className="form-group" style={{ marginBottom: 0 }}>
                    <label htmlFor="withdraw-amount" style={{ fontWeight: 700, marginBottom: 6, display: 'block' }}>Amount</label>
                    <div style={{ position: 'relative', width: '100%' }}>
                      <span style={{ position: 'absolute', left: 18, top: '50%', transform: 'translateY(-50%)', color: '#1976d2', fontWeight: 700, fontSize: '1.13em', pointerEvents: 'none', letterSpacing: 0.5 }}>ETB</span>
                      <input
                        id="withdraw-amount"
                  ...
                  {/* Payment Method */}
                  <div className="form-group" style={{ marginBottom: 0 }}>
                    <label style={{ fontWeight: 700, marginBottom: 6, display: 'block' }}>Withdrawal Method</label>
                    <div className="modal-methods" style={{ gap: 12 }}>
                      <button type="button" className={`modal-method-btn${fundMethod === 'bank' ? ' active' : ''}`} style={{ minWidth: 110 }} onClick={() => setFundMethod('bank')}><i className="fas fa-university"></i> Bank</button>
                      <button type="button" className={`modal-method-btn${fundMethod === 'mobile' ? ' active' : ''}`} style={{ minWidth: 120 }} onClick={() => setFundMethod('mobile')}><i className="fas fa-mobile-alt"></i> Mobile Money</button>
                    </div>
                  ...
                  {fundMethod === 'bank' && (
                    <div className="form-group" style={{ marginBottom: 0 }}>
                      <label htmlFor="bank-ref" style={{ fontWeight: 700, marginBottom: 6, display: 'block' }}>Bank Account Number</label>
                      <input
                        id="bank-ref"
                        className="modal-input"
                        value={bankRef}
                  ...
                  <div style={{ position: 'sticky', bottom: 0, background: 'white', paddingTop: 18, paddingBottom: 0, zIndex: 1, margin: 0 }}>
                    <button className="btn btn-add-fund-main modal-btn" type="submit" disabled={!fundAmount} style={{ width: '100%', fontWeight: 800, fontSize: '1.15em', padding: '15px 0', borderRadius: 10, boxShadow: '0 2px 12px rgba(0,123,255,0.10)', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: 0 }}>
                      <i className="fas fa-arrow-up" style={{ marginRight: 8 }}></i> Withdraw Funds
                    </button>
                  </div>
                </form>
              )}
